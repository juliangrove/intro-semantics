...
Said once more and in another way:
...
Both presentations say exactly the same thing.
...
This is a function that maps *numerals* onto *natural numbers*---coincidentally, just like the interpretation function does!
That is:

(@ex-length-function) $length : n → ℕ$

But of course, aside from its domain and codomain, it does not behave like the interpretation function at all.
...
(@ex-ana-proofs) 1. \begin{prooftree}\AxiomC{$⟨⌜0⌝, I_{ANA}(⌜0⌝)⟩ ⊢ atom$}\end{prooftree} <br>
...
It is worth---that is, I highly recommend---reading each of the rules in (@ex-ana-proofs) carefully and comparing it to its corresponding rule in (@ex-ana-semantics), in order to convince yourself that the two versions of the rule say exactly the same thing.
Note that the closure clause is only implied on the proof-based presentation.

...
For example, consider the following sentences, in which sentences that normally display presuppositions are made the consequent clauses of certain conditional sentences:
...
(@ex-def-conditional) does *not* imply that there is a dog, nor does (@ex-love-conditional) imply that Jo is good at Go.

We obtain a similar pattern if we conjoin the antecedent sentences of these conditionals with the corresponding consequent sentences, and then negate the sentences that result:
...
This sounds quite odd, suggesting that the conventional implicature is forced to project;
that is, what may make this conditional sentence odd is that it triggers the inference that Julian enjoys syntax---due to the relative clause---while at the same time, the fact that *Julian enjoys syntax* is the antecedent the conditional may trigger an ignorance implicature, i.e., that the utterer doesn't know whether or not Julian enjoys syntax.
...
For a given candidate presupposition or conventional implicature $S$, can you place $S$ into the consequent of a conditional sentence in the way described above, so that it is canceled?

...
Since our general goal is to give an account of conventional meaning---the type of meaning that users of a language associate with expressions by *convention* in a way that allows them to deploy the expressions to communicate in real conversational settings---it would be useful to first get a handle on some general conversational principles about *how* expressions with certain meanings are deployed.
Doing this can help us determine---from the inferences that we observe in real-life language use---which of those inferences are related to conventional meaning and which are related to general (and perhaps, to some extent, non-linguistic) social conventions about conversation itself.

With this aim partly in mind, @grice_logic_1975 proposes a principle that he takes to regulate real conversation:
...
But we can still get a certain amount of reasoning about why certain utterances mean what they do off the ground, even with while leaving a lot underspecified.
...
That is, don't be like [zombie kid](https://www.youtube.com/watch?v=CMNry4PE93Y&t=2s).
...
it can be viewed as saying that one should make statements that answer the QUD, at least partially.
